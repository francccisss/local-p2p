...
Initial Seeders will be required to generate a hash for their own target file that they wish to be public for other nodes in the network. This design removes the need for the DHT server to do anything else other than to just server a file's meta data to other nodes that wishes to connect.

! [Initial Seeding of File](./docs/initial-seeder.jpeg)

the `FileMetaData` will be generated by calling the function `NewFileMetaData` that handles, content hash generation using `sha256` as well as extending the target file's attribute by embedding the hash directly into the target file's meta data field
