...
// TODO:
// `srcPath` refers to the existing file in that file path eg: /path/to/file.exe
// `dstPath` refers to the same thing
// func UploadFile(srcPath string, dstPath string) {
// }
// called by Upload File
// func GetFileHashID(IDLength int) FileHash {
//
// }
...
func EmbbedFileHashID(hash FileHash, src string) error {
	fmt.Printf("[ EMBEDDING ]: Checking if the file '%s' Exists\n", src)
	f, err := os.OpenFile(src, os.O_WRONLY, 0644)
	if err != nil {
		fmt.Println("[ EMBEDDING ]: Unable to Open File")
		return err
	}
	fmt.Printf("[ EMBEDDING ]: The file '%s' Exists\n", src)
	fmt.Printf("[ EMBEDDING ]: Embedding HashID to '%s'\n", src)
	err = unix.Fsetxattr(int(f.Fd()), ATTRIBUTE_STRING, []byte(hash), 0)
	if err != nil {
